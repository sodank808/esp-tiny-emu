...
static void play_click(espp::EspWaveshare &bsp);
...
extern "C" void app_main(void) {
  ...
  //! [esp box example]
  espp::EspWaveshare &bsp = espp::EspWaveshare::get();
  bsp.set_log_level(espp::Logger::Verbosity::INFO);
  logger.info("Running on {}", bsp.box_type());
  ...
  using Imu = espp::EspWaveshare::Imu;
  ...
  espp::Task imu_task(
      {.callback = [&](std::mutex &m, std::condition_variable &cv) -> bool {
         ...
         static auto &bsp = espp::EspWaveshare::get();
