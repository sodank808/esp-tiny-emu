...
class EspWaveshare : public BaseComponent {
public:
  ...
  static EspWaveshare &get() {
    static EspWaveshare instance;
    return instance;
  }

  EspWaveshare(const EspWaveshare &) = delete;
  EspWaveshare &operator=(const EspWaveshare &) = delete;
  EspWaveshare(EspWaveshare &&) = delete;
  EspWaveshare &operator=(EspWaveshare &&) = delete;
...
template <> struct fmt::formatter<espp::EspWaveshare::BoxType> : fmt::formatter<std::string> {
  template <typename FormatContext> auto format(espp::EspWaveshare::BoxType c, FormatContext &ctx) const {
    std::string name;
    switch (c) {
    case espp::EspWaveshare::BoxType::UNKNOWN:
      name = "UNKNOWN";
      break;
    case espp::EspWaveshare::BoxType::BOX:
      name = "BOX";
      break;
    case espp::EspWaveshare::BoxType::BOX3:
      name = "BOX3";
      break;
    }
