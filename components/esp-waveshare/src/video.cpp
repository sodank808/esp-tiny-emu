...
static void lcd_spi_pre_transfer_callback(spi_transaction_t *t) {
  static auto lcd_dc_io = EspWaveshare::get_lcd_dc_gpio();
...
bool EspWaveshare::initialize_lcd() {
  if (lcd_handle_) {
  ...
  DisplayDriver::initialize(espp::display_drivers::Config{
      .write_command = std::bind(&EspWaveshare::write_command, this, _1, _2, _3),
      .lcd_send_lines = std::bind(&EspWaveshare::write_lcd_lines, this, _1, _2, _3, _4, _5, _6),
...
bool EspWaveshare::initialize_display(size_t pixel_buffer_size) {
  if (!lcd_handle_) {
    logger_.error(
...
void EspWaveshare::lcd_wait_lines() {
...
void EspWaveshare::write_command(uint8_t command, std::span<const uint8_t> parameters,
                           uint32_t user_data) {
  lcd_wait_lines();
...
void EspWaveshare::write_lcd_lines(int xs, int ys, int xe, int ye, const uint8_t *data,
                             uint32_t user_data) {
...
void EspWaveshare::write_lcd_frame(const uint16_t xs, const uint16_t ys, const uint16_t width,
...
EspWaveshare::Pixel *EspWaveshare::vram0() const {
  if (!display_) {
    return nullptr;
  }
...
EspWaveshare::Pixel *EspWaveshare::vram1() const {
  if (!display_) {
    return nullptr;
  }
...
uint8_t *EspWaveshare::frame_buffer0() const { return frame_buffer0_; }

uint8_t *EspWaveshare::frame_buffer1() const { return frame_buffer1_; }

void EspWaveshare::brightness(float brightness) {
...
float EspWaveshare::brightness() const {
