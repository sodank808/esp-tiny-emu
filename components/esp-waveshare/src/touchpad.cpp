...
bool EspWaveshare::initialize_touch(const EspWaveshare::touch_callback_t &callback) {
  if (touchpad_input_) {
  ...
  touchpad_input_ = std::make_shared<espp::TouchpadInput>(espp::TouchpadInput::Config{
      .touchpad_read =
          std::bind(&EspWaveshare::touchpad_read, this, std::placeholders::_1, std::placeholders::_2,
...
bool EspWaveshare::update_gt911() {
...
bool EspWaveshare::update_tt21100() {
...
bool EspWaveshare::update_touch() {
  switch (box_type_) {
  case BoxType::BOX3:
    return update_gt911();
  case BoxType::BOX:
...
void EspWaveshare::touchpad_read(uint8_t *num_touch_points, uint16_t *x, uint16_t *y,
                           uint8_t *btn_state) {
  std::lock_guard<std::recursive_mutex> lock(touchpad_data_mutex_);
...
EspWaveshare::TouchpadData EspWaveshare::touchpad_convert(const EspWaveshare::TouchpadData &data) const {
  TouchpadData temp_data;
