...
bool EspWaveshare::initialize_codec() {
...
bool EspWaveshare::initialize_i2s(uint32_t default_audio_rate) {
...
bool EspWaveshare::initialize_sound(uint32_t default_audio_rate,
  ...
  audio_task_ = espp::Task::make_unique({
      .callback = std::bind(&EspWaveshare::audio_task_callback, this, _1, _2, _3),
...
void EspWaveshare::enable_sound(bool enable) { gpio_set_level(sound_power_pin, enable); }

bool EspWaveshare::audio_task_callback(std::mutex &m, std::condition_variable &cv, bool &task_notified) {
...
void EspWaveshare::update_volume_output() {
  if (!sound_initialized_) {
    return;
  }
  if (mute_) {
...
void EspWaveshare::mute(bool mute) {
  mute_ = mute;
  update_volume_output();
}

bool EspWaveshare::is_muted() const { return mute_; }

void EspWaveshare::volume(float volume) {
  volume_ = volume;
  update_volume_output();
}

float EspWaveshare::volume() const { return volume_; }

uint32_t EspWaveshare::audio_sample_rate() const { return audio_std_cfg.clk_cfg.sample_rate_hz; }

size_t EspWaveshare::audio_buffer_size() const { return audio_tx_buffer.size(); }

void EspWaveshare::audio_sample_rate(uint32_t sample_rate) {
...
void EspWaveshare::play_audio(const std::vector<uint8_t> &data) { play_audio(data.data(), data.size()); }

void EspWaveshare::play_audio(const uint8_t *data, uint32_t num_bytes) {
  play_audio_task_handle_ = xTaskGetCurrentTaskHandle();
  if (has_sound) {
